...
export function CarruselF({dataUser}) {

  const { postsF, getPosts, addAcceptedPost } = usePosts();
  ...
  const handleAccept = (e, idPost, idUserFree, idUserEmployee) => {
    e.preventDefault()
    let data = {
        id_pf: idUserFree,
        id_pe: idUserEmployee,
        id_post: idPost
    }
    addAcceptedPost(data);
  }
  ...
  <>
            ...
            <Card className="h-auto">
              <CardContent className="p-4">
                ...
                <div className="flex flex-wrap gap-1">
                  ...
                  <Button
                    className="h-6 w-16 hover:bg-[#32a742]"
                    onClick = {(e) => handleAccept(e, posts.id, dataUser.id, posts.id_perfil)}
                  >
                    Aceptar
                  </Button>
                </div>
    ...
    : (<div className="flex justify-center text-2xl">No hay post aun D:</div> 
    )}
  </>
  );
}