...
import { CarruselF } from "./Carrusel/CarruselF";
import { CarruselAF } from "./Carrusel/CarruselAF";

export function HomePageFreelancer() {
  const navigate = useNavigate();
  const [dataUser, setDataUser] = useState("");
  const { updateProfileF } = usePosts();
    ...
    const getUser = async () => {
      ...
      if (!user) {
        ...
        if (error) {
          ...
          navigate("/CreateProfileF", { state: { userId: user.id } });
        } else {
          setDataUser(data[0]);
        }
      }
  ...
  const handleUpdate = (event) => {
    ...
    const { error } = updateProfileF(dataUser.id, updatedProfile);
  ...
  return (
    <div className="relative  flex flex-col min-h-screen">
      <header className="flex items-center justify-between p-4 bg-primary text-primary-foreground">
        <div className="flex items-center space-x-4">
          <Avatar>
            ...
            <AvatarFallback className="text-cyan-950 font-bold">
              {dataUser ? dataUser.Usuario[0] : "X"}
            </AvatarFallback>
          </Avatar>
          <span className="text-lg font-semibold">
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle>
                    Perfil De {dataUser.Usuario}
                  </AlertDialogTitle>
                  <AlertDialogDescription>
                    Esta es tu informacion como Freelancer
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <form onSubmit={handleUpdate} className="grid-cols-1 gap-1">
                  <Label>Nombre de usuario</Label>
                  <Input name="Usuario" placeholder={dataUser.Usuario} />
                  <Label>Tecnologias</Label>
                  <Textarea
                    name="Tecnologias"
                    placeholder={dataUser.Tecnologias}
                  />
                  <Label>Proyectos</Label>
                  <Input name="Proyectos" placeholder={dataUser.Proyectos} />

                  <AlertDialogFooter className="p-3 ">
                    <AlertDialogCancel>Cerrar</AlertDialogCancel>
                    <AlertDialogAction
                      type="submit"
                      className="hover:bg-[#bba737]"
                    >
                      Actualizar
                    </AlertDialogAction>
                  </AlertDialogFooter>
        ...
        <div className="text-3xl font-bold text-center">
          FreeIn <p className="text-sm">(Freelancer)</p>
        </div>
      </header>
      <main className="flex-grow p-6 bg-background">
        ...
        <CarruselF dataUser={dataUser}/>
        <section className="p-3">
          <h2 className="text-2xl font-bold my-3">Posts Aceptados</h2>
        </section>
        <CarruselAF />
      </main>
